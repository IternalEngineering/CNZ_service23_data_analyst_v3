...
# -*- coding: utf-8 -*-
"""
City Insights Analyzer - OpenAI Version
Uses OpenAI agent to analyze city data across disparate sources,
...
class CityInsightsAnalyzerOpenAI:
    """Analyzer that uses OpenAI to generate insights from disparate data sources"""

    def __init__(self, trace_context: Optional[dict] = None):
        """
        Initialize the analyzer

        Args:
            trace_context: Optional trace context from parent service for trace propagation
        """
    ...
    async def analyze_city(
        ...
        """
        # Restore parent trace context if provided (for cross-service trace propagation)
        ctx = context.get_current()
        if self.trace_context:
            # This allows the span to be part of the parent trace
            ctx = self.trace_context

        # Create agent span for Arize agent graph visualization with proper hierarchy
        with tracer.start_as_current_span(
            name="SQL Query Generator Agent",
            context=ctx,
            attributes={
                "graph.node.id": "sql_query_generator",
                "graph.node.parent_id": "service23_data_analysis",
                "agent.role": "sql_generator",
                "agent.task": "generate_data_queries",
...
Remember: Use LIMIT in ALL queries. Query at most 5 times total."""
            ...
            try:
                while iteration < max_iterations:
                    ...
                    if len(messages) > 8:  # Keep system + last 6 messages
                        print(f"  (Pruning conversation: {len(messages)} -> 8 messages)")
                    ...
                    try:
                        ...
                        if "too long" in error_msg.lower() or "tokens" in error_msg.lower():
                            print(f"  Context overflow detected, pruning more aggressively...")
                            messages = [messages[0]] + messages[-2:]
                            continue

                        # Unhandled exception - record error and re-raise
                        sql_span.set_status(Status(StatusCode.ERROR, error_msg))
                        sql_span.set_attribute("error.type", type(e).__name__)
                        sql_span.set_attribute("error.message", error_msg)
                        raise
                    ...
                    if message.tool_calls:
                        query_count += len(message.tool_calls)
                        ...
                        # Now create a second span for the insights synthesis
                        with tracer.start_as_current_span(
                            name="Data Insights Synthesizer Agent",
                            context=ctx,
                            attributes={
                                "graph.node.id": "insights_synthesizer",
                                "graph.node.parent_id": "service23_data_analysis",
                                "agent.role": "insights_analyst",
                                "agent.task": "synthesize_insights_from_data",
                                "agent.model": self.model,
                                "city": city,
                                "country_code": country_code,
                            ...
                            try:
                                ...
                                # Mark both spans as successful
                                sql_span.set_status(Status(StatusCode.OK))
                                sql_span.set_attribute("queries_executed", query_count)

                                insights_span.set_status(Status(StatusCode.OK))
                                insights_span.set_attribute("confidence_score", insight_data.get("confidence_score", 0.5))
                                insights_span.set_attribute("recommendations_count", len(insight_data.get("recommendations", [])))
                                ...
                                insights_span.set_status(Status(StatusCode.ERROR, "JSON parse error"))
                                sql_span.set_status(Status(StatusCode.OK))
                                sql_span.set_attribute("queries_executed", query_count)
                ...
                sql_span.set_status(Status(StatusCode.ERROR, "Max iterations reached"))
                sql_span.set_attribute("iterations", iteration)
                ...
                # Ensure span error status is set
                sql_span.set_status(Status(StatusCode.ERROR, str(e)))
                sql_span.set_attribute("error.type", type(e).__name__)
                raise
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze city data and generate insights (OpenAI version)")
    ...
    analyzer = CityInsightsAnalyzerOpenAI()
